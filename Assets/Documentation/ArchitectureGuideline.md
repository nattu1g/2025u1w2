...
### 1. Presenterの責務分割と`EntryPoint`の活用

*   **原則:** `MainPresenter`のような単一の巨大なPresenterに責務を集中させるのではなく、**機能単位（画面単位）で`Presenter`を分割**します。（例：`OptionPresenter`, `PlayerClubPresenter`など）
*   **実装:** 分割した各`Presenter`を、`MainPresenter`と同様に`LifeTimeScope`で**`EntryPoint`として登録**します。これにより、`Presenter`同士は親子関係ではなく、DIコンテナに管理される対等な関係になります。
*   **初期化順序:** `EntryPoint`間の初期化順序を制御する必要がある場合は、`builder.RegisterEntryPoint<T>().WithOrder(int)`を使い、`LifeTimeScope`上で明示的に順序を定義します。

**メリット:** 各`Presenter`が自身の関心事に集中でき、`MainPresenter`の肥大化を防ぎます。機能追加・修正が他の部分に影響を与えにくくなり、コードの見通しが格段に良くなります。
